...
function connectToMqtt() {
  ...
  function onConnect() {
    ...
    client.send(message);
    client.subscribe(
      "iot_vertical_farming_ppu2022/centric/manual_big_pump1_water_flow"
    );
    client.subscribe(
      "iot_vertical_farming_ppu2022/centric/manual_small_pump1_acid"
    );
    client.subscribe(
      "iot_vertical_farming_ppu2022/centric/manual_small_pump2_base"
    );
    client.subscribe(
      "iot_vertical_farming_ppu2022/centric/manual_small_pump3_nutrient"
    );
    ...
    client.subscribe(
      "iot_vertical_farming_ppu2022/RPI/update_firebase_range_value"
    );
  }
  ...
  function onMessageArrived(message) {
    ...
    if (topics.includes(message.topic)) {
      ...
      const obj = JSON.parse(message.payloadString, function (key, value) {
        let elem = document.getElementById(message.topic + key);
        if (elem != null) {
...
function subscribe() {
  sendMessage(
    '{ "air_temperature": "30", "air_humidity": "52", "light_intensity": "900" }',
    "iot_vertical_farming_ppu2022/branch2/sensor_value"
  ); // code goes here
}
